...
unsigned char BitLib::bitSetChar(unsigned char number, unsigned char index, bool bit) {
    if (bit) {
...
unsigned char BitLib::bitSetChar(unsigned int number, unsigned char index, bool bit) {
    if (bit) {
        return number | (1 << index);
    }
    else {
...
unsigned int BitLib::bitSetInt(unsigned int number, unsigned char index, bool bit) {
    if (bit) {
        return number | (1 << index);
    }
    else {
